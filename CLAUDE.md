...
- **새 ID 부여 전 반드시 grep** — 해당 `.sdoc` 안에서 `UID: <PREFIX>###` 로 검색해 비어 있는지 확인한 뒤, 그 파일의 최대치 +1 로 부여. 여러 요구사항을 동시에 추가할 때는 번호 충돌 특히 주의.
- 코드 주석(`// SWR016`)과 1:1 대응되므로 번호가 변하면 전 레포 grep 필요.
